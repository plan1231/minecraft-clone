...
void PhysicsSystem::update(float dt) {
    registry.view<Dynamics, AABB, Transform, CharacterMovement>().each(
            [&](Dynamics &dyn, AABB &aabb, Transform &transform, CharacterMovement &cm) {
                ...
                if (dyn.acceleration.x == 0.0f && dyn.acceleration.z == 0.0f && xzSpeed > 0.0f) {
                    ...
                    dyn.velocity.x -=
                            dyn.velocity.x > 0 ? std::min(xzNormalized.x * DRAG_DEACCELERATION * dt, dyn.velocity.x)
                                               : std::max(xzNormalized.x * DRAG_DEACCELERATION * dt, dyn.velocity.x);
                    dyn.velocity.z -=
                            dyn.velocity.z > 0 ? std::min(xzNormalized.y * DRAG_DEACCELERATION * dt, dyn.velocity.z)
                                               : std::max(xzNormalized.y * DRAG_DEACCELERATION * dt, dyn.velocity.z);
                }
                        ...
                        threadPool.queueJob([this, y, &minInt, &maxInt, &speedZ, &speedX, &aabb, &adjVel, &collisions, &vecLock]() {
                            ...
                            for (int x = minInt.x - 4 - speedX; x <= maxInt.x + 4 + speedX; x++) {
                                for (int z = minInt.z - 4 - speedZ; z <= maxInt.z + 4 + speedZ; z++) {
                                    ...
                                    perThreadCollisions.emplace_back(cTime, normal, glm::ivec3{x, y, z});
                                }
                            }
                            if (!perThreadCollisions.empty()) {
                                std::lock_guard<std::mutex> lock(vecLock);
                                collisions.push_back(*std::ranges::min_element(perThreadCollisions,
                                                                           [](const std::tuple<float, glm::vec3, glm::ivec3> &lhs,
